...
export async function createCertificates(
  ...
  try {
    const validateData = createCertificatesSchema.safeParse(input);
    ...
    const newData = await db.certificates.create({
      data: {
...
export async function uptadeCertificates(
  id: string,
  ...
  try {
    ...
    const parsedInput = uptadeCertificatesSchema.safeParse(input);
...
export async function uptadeCertificatesImage(
  id: string,
  input: ImgInput
): Promise<ActionResult> {
  const { user, error } = await checkAuthServerAction([
  ...
  try {
    ...
    const uptadeData = await db.certificates.update({
      where: { documentId: id },
      data: {
