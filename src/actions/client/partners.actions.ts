...
export async function uptadePartners(
  id: string,
  ...
  try {
    const existingCategory = await db.partners.findUnique({
      where: {
        documentId: id,
        isDeleted: false,
      },
    ...
    const uptadeData = await db.$transaction(async (prisma) => {
      const updatedData = await prisma.partners.update({
        where: { documentId: id },
        data: {
          url,
          translations: {
...
export async function uptadePartnersImage(
  id: string,
  input: ImgInput
): Promise<ActionResult> {
  const { user, error } = await checkAuthServerAction([
  ...
  try {
    ...
    const uptadeData = await db.partners.update({
      where: { documentId: id },
      data: {
        imageId: Number(imageId),
      },
