...
import {
  ...
  UpdateContactEnumInput,
  uptadeContactEnumSchema,
...
export async function getContactEnumData({
  page,
  pageSize,
  query,
  locale,
  ...
  const skip = 0;
  const take = Number(defaultPage) * customPageSize;
  const searchTerm = query?.trim();
  const whereClause: Prisma.ContactEnumWhereInput = {
    isDeleted: false,
    translations: {
      some: {
        locale: locale,
  ...
  const [data, totalCount] = await Promise.all([
    ...
    db.contactEnum.count({ where: whereClause }),
  ]);
...
export async function getContactEnumById({ locale, id }: GetByIDProps) {
  try {
    ...
    const existingData = await db.contactEnum.findFirst({
      where: whereClause,
      select: {
        translations: {
          where: { locale },
...
export async function createConactEnum(
  input: CreateContactEnumInput
  ...
  try {
    const validateData = createContactEnumSchema.safeParse(input);
    ...
    const newData = await db.contactEnum.create({
      data: {
        translations: {
          create: {
            title: title,
...
export async function uptadeContactEnum(
  id: string,
  input: UpdateContactEnumInput
): Promise<ActionResult> {
  const { user, error } = await checkAuthServerAction([
    Role.ADMIN,
    Role.SUPER_ADMIN,
  ...
  try {
    const existingData = await db.contactEnum.findUnique({
      where: {
        documentId: id,
        isDeleted: false,
      },
    ...
    const parsedInput = uptadeContactEnumSchema.safeParse(input);
    ...
    const uptadeData = await db.$transaction(async (prisma) => {
      const updatedData = await prisma.contactEnum.update({
        where: { documentId: id },
        data: {
          translations: {
            upsert: {
