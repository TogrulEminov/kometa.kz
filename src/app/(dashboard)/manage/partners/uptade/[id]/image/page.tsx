...
import { uptadeAdvantagesImage } from "@/src/actions/client/advantages.actions";
import { getPartnersById } from "@/src/actions/client/partners.actions";

type UpdateAdvantagesImageInput = z.infer<typeof imgSchema>;
...
export default function AdvantagesUpdateImagePage() {
  ...
  } = useForm<UpdateAdvantagesImageInput>({
  ...
  const { mutate: deleteFile } = useDeleteData<FileType>({
    invalidateKeys: ["advantages", `advantages/${id}`],
  });

  const onSubmit = async (data: UpdateAdvantagesImageInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await uptadeAdvantagesImage(existingData.documentId, {
