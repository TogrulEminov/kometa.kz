...
import { createAdvantages } from "@/src/actions/client/advantages.actions";
...
export default function CreateAdvantages() {
  ...
  const {
    ...
    watch,
    reset,
  } = useForm<CreateAdvantagesInput>({
    resolver: zodResolver(createAdvantagesSchema),
    defaultValues: {
      title: "",
      description: "",
      imageId: "",
  ...
  const onSubmit = async (data: CreateAdvantagesInput) => {
    startTransition(async () => {
      const result = await createAdvantages({
        ...data,
