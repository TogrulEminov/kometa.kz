...
import { About, FileType, UploadedFileMeta } from "@/src/services/interface";
import { useDeleteData } from "@/src/hooks/useApi";
import { useState } from "react";
import { ImgInput, imgSchema } from "@/src/schema/img.schema";
import { useMessageStore } from "@/src/hooks/useMessageStore";
import { updateAboutImage } from "@/src/actions/client/about.actions";
...
export default function AboutUpdateImagePage({ existingData, refetch }: Props) {
  ...
  const onSubmit = async (data: ImgInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await updateAboutImage(existingData.documentId, {
