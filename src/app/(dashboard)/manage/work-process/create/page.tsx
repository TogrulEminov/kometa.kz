...
import { createWorkProcess } from "@/src/actions/client/workprocess.actions";

export default function CreateWorkProcess() {
  ...
  const {
    ...
    watch,
    reset,
  } = useForm<CreateWorkProcessInput>({
    resolver: zodResolver(createWorkProcessSchema),
    defaultValues: {
      title: "",
      description: "",
      orderNumber: 0,
  ...
  const onSubmit = async (data: CreateWorkProcessInput) => {
    startTransition(async () => {
      const result = await createWorkProcess({
        ...data,
