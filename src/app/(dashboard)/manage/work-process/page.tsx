...
import { getWorkProcessData } from "@/src/actions/client/workprocess.actions";

export default function WorkProcessAdminPage() {
  ...
  const columns: Column<WorkProcessItem>[] = [
    createImageTitleColumn<WorkProcessItem>(),
    createStatusColumn<any>(),
    createCreatedAtColumn<WorkProcessItem>(),
    createUpdatedAtColumn<WorkProcessItem>(),
  ];
  ...
  return (
    <div>
      <WhiteBlockTitleArea
        title="iş Procesi"
        link="/manage/work-process/create?locale=az"
      />
      <SearchingArea link="manage/work-process" />
      <Suspense
        fallback={
          <div
            style={{
      ...
      >
        <TableTypesArea<WorkProcessItem>
          columns={columns}
          page="work-process"
          model="work-process"
          dataItems={(data?.data as unknown as WorkProcessItem[]) || []}
          isError={isError}
          refetch={refetch}
          locale={locale}
          invalidateQueryKey="work-process"
          isLoading={isLoading}
        />
      </Suspense>
