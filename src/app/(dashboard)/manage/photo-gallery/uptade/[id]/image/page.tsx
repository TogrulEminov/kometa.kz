...
export default function PhotoesUpdateImagePage() {
  ...
  const { data: existingData, refetch } = useServerQueryById<GalleryItem>(
    `gallery-admin`,
    getCategoryWrapper,
    id,
    { locale: "az" }
  ...
  const { mutate: deleteFile } = useDeleteData<FileType>({
    invalidateKeys: ["gallery-admin", `gallery-admin/${id}`],
  });
  ...
  const onSubmit = async (data: UpdateYoutubeImageInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await uptadePhotoImage(existingData.documentId, {
