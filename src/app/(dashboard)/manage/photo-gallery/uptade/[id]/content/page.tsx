...
import { CustomLocales, GalleryItem } from "@/src/services/interface";
...
import {
  getGalleryById,
  uptadeGalleryContent,
} from "@/src/actions/client/gallery.actions";
import {
  UpdatePhotoesInput,
  uptadePhotoesSchema,
...
export default function UptadeContent() {
  ...
  const getDataWrapper = async () => {
    const result = await getGalleryById({
      locale: locale as CustomLocales,
      id: id as string,
    });

    return {
      data: result.data as GalleryItem | undefined,
      message: result.message,
      code: result.code,
    };
  };
  const { data: existingData } = useServerQueryById<GalleryItem>(
    `gallery-admin`,
    getDataWrapper,
    id,
    { locale }
  ...
  const {
    ...
    reset,
  } = useForm<UpdatePhotoesInput>({
    resolver: zodResolver(uptadePhotoesSchema),
    values: {
  ...
  const onSubmit = async (data: UpdatePhotoesInput) => {
    startTransition(async () => {
      const result = await uptadeGalleryContent(id as string, data);
