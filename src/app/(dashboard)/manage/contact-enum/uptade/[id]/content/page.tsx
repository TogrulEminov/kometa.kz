...
import {
  getContactEnumById,
  uptadeContactEnum,
} from "@/src/actions/client/contactEnum.actions";
import {
  UpdateContactEnumInput,
  uptadeContactEnumSchema,
} from "@/src/schema/contactEnum.schema";

export default function ContactUptadeContent() {
  ...
  const getDataWrapper = async () => {
    const result = await getContactEnumById({
  ...
  const {
    ...
    reset,
  } = useForm<UpdateContactEnumInput>({
    resolver: zodResolver(uptadeContactEnumSchema),
    values: {
  ...
  const onSubmit = async (data: UpdateContactEnumInput) => {
    startTransition(async () => {
      const result = await uptadeContactEnum(id as string, {
        ...data,
      });
