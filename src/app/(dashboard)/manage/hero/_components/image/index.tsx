...
import { FileType, Hero, UploadedFileMeta } from "@/src/services/interface";
import { useDeleteData } from "@/src/hooks/useApi";
import { z } from "zod";
import { useState } from "react";
import { imgSchema } from "@/src/schema/img.schema";
import { useMessageStore } from "@/src/hooks/useMessageStore";
import { updateHeroImage } from "@/src/actions/client/hero.actions";

type uptadeHeroImageInput = z.infer<typeof imgSchema>;
interface Props {
  existingData: Hero | undefined;
  refetch: () => void;
}
export default function HeroUpdateImagePage({ existingData, refetch }: Props) {
  ...
  const { mutate: deleteFile } = useDeleteData<FileType>({
    invalidateKeys: ["hero", `hero/${id}`],
  });

  const onSubmit = async (data: uptadeHeroImageInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await updateHeroImage(existingData.documentId, {
