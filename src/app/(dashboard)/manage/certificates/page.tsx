"use client";
import React, { Suspense } from "react";
import { usePaginationQuery } from "@/src/hooks/usePaginationQuery";
import { Pagination } from "antd";
import WhiteBlockTitleArea from "../_components/whiteBlockTitle";
import SearchingArea from "../_components/whiteBlockSearch";
import TableTypesArea from "../_components/table";
import { Certificates, Column, CustomLocales } from "@/src/services/interface";
import {
  createCreatedAtColumn,
  createImageColumn,
  createImageTitleColumn,
  createStatusColumn,
  createUpdatedAtColumn,
} from "@/src/utils/tableColumns";
import { Spin } from "antd";
import { useServerQuery } from "@/src/hooks/useServerActions";
import { getCertificates } from "@/src/actions/client/certificates.actions";

export default function AdminCertificatesPage() {
  const { queryParams, handleChange, locale } = usePaginationQuery();
  const { data, isLoading, isError, refetch } = useServerQuery(
    "youtube",
    getCertificates,
    {
      params: {
        page: queryParams.page,
        pageSize: queryParams.pageSize,
        query: queryParams.title,
        locale: locale as CustomLocales,
      },
    }
  );

  const totalCount = Number(data?.paginations?.dataCount) || 0;
  const page = Number(data?.paginations?.page) || 1;
  const pageSize = Number(data?.paginations?.pageSize) || 12;
  const totalPages = Number(data?.paginations?.totalPages) || 1;

  const columns: Column<Certificates>[] = [
    createImageTitleColumn<Certificates>(),
    createStatusColumn<any>(),
    createCreatedAtColumn<Certificates>(),
    createUpdatedAtColumn<Certificates>(),
    createImageColumn<Certificates>({
      page: "certificates",
    }) as Column<Certificates>,
  ];

  return (
    <div>
      <WhiteBlockTitleArea
        title="Sertifikatlar"
        link="/manage/certificates/create?locale=az"
      />
      <SearchingArea link="manage/certificates" />
      <Suspense
        fallback={
          <div
            style={{
              display: "flex",
              justifyContent: "center",
              padding: "50px",
            }}
          >
            <Spin size="large" />
          </div>
        }
      >
        <TableTypesArea<Certificates>
          columns={columns}
          page="certificates"
          model="certificates"
          dataItems={(data?.data as unknown as Certificates[]) || []}
          isError={isError}
          refetch={refetch}
          locale={locale}
          invalidateQueryKey="certificates"
          isLoading={isLoading}
        />
      </Suspense>

      {totalPages > 1 && (
        <div style={{ marginTop: "40px" }}>
          <Pagination
            current={page}
            total={totalCount}
            pageSize={pageSize}
            onChange={handleChange}
          />
        </div>
      )}
    </div>
  );
}
