...
import {
  getCertificatesById,
  uptadeCertificatesImage,
} from "@/src/actions/client/certificates.actions";

type UpdateDataImageInput = z.infer<typeof imgSchema>;

export default function CertificatesUpdateImagePage() {
  ...
  const getCategoryWrapper = async () => {
    ...
    return {
      data: result.data as Certificates | undefined,
      message: result.message,
      code: result.code,
    };
  };

  const { data: existingData, refetch } = useServerQueryById<Certificates>(
    `certificates`,
    getCategoryWrapper,
    id,
    { locale: "az" }
  ...
  const { mutate: deleteFile } = useDeleteData<FileType>({
    invalidateKeys: ["youtube", `youtube/${id}`],
  });

  const onSubmit = async (data: UpdateDataImageInput) => {
    ...
    startTransition(async () => {
      try {
        const result = await uptadeCertificatesImage(existingData.documentId, {
