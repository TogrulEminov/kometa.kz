...
import { Certificates, CustomLocales } from "@/src/services/interface";
...
} from "@/src/schema/certificates.schema";

export default function CertificatesUptadeContent() {
  ...
  const { data: existingData } = useServerQueryById<Certificates>(
    `certificates`,
    getCategoryWrapper,
    id,
    { locale }
  ...
  const {
    ...
    watch,
    reset,
  } = useForm<UpdateCertificatesInput>({
  ...
  const onSubmit = async (data: UpdateCertificatesInput) => {
    startTransition(async () => {
      const result = await uptadeCertificates(id as string, {
        ...data,
      });
